...
int	pro_exec2()
{
	...
	pn2_nobj = malloc ( MAX_NOBJ * sizeof( pn1_nobj ) );
	if (pn2_nobj == NULL)
		error(908);


	for (i=0; i<MAX_NOBJ; i++)
	{
		pn2_nobj[i] = malloc ( sizeof ( str_nobj ) );
		if ( pn2_nobj[i] == NULL)
			error(908);
	}
	...
	for (i=0; i<MAX_NOBJ; i++)
	{
		memcpy ( &s1, pn2_nobj[i], sizeof( s1 ) );


		printf ("i: %4d  c1: %4d (v1)  c1: %4d (v2) %s\n",
			i,
			f1(i).nobj_c2,
			s1.nobj_c2,
			" ");
